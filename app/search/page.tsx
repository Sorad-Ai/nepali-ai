        ...
        {loading ? (
          ...
          visibleCards.map((card, index) => {
            const cardElement = (
              ...
              >
                <Image className="card-img" src={card.img} alt={card.title} width={300} height={200} />
                <h3 className="card-title" style={{ fontSize: '15px' }}>{card.title}</h3>
              </Link>
            );
            ...
            return (
              <div key={card.sn} className='cards' style={{ height: '150px' }}>
                {cardElement}
              </div>
            );
          })
